...
functionDefinition returns[FunctionDefinition fdRet]
    ...
    dr=declarator { $fdRet.setDeclarator($dr.drRet); $fdRet.setLine($dr.drRet.getLine()); } (dl=declarationList { $fdRet.setDeclarationList($dl.dlRet); })?
...
declarator returns[Declarator drRet]
    : { $drRet = new Declarator(); }
      ...
      dtdr=directDeclarator { $drRet.setDd($dtdr.dd); $drRet.setLine($dtdr.dd.getLine()); } ;
...
directDeclarator returns [DirectDeclarator dd]
    ...
    | lp=LeftParen dr=declarator RightParen {
        $dd = new DirectDeclarator();
        $dd.setDr($dr.drRet);
        $dd.setLine($lp.line);
      }
    | ddr=directDeclarator { $dd = new DirectDeclarator(); $dd.setDd($ddr.dd); $dd.setLine($ddr.dd.getLine());} LeftBracket (e=expression {$dd.setE($e.exprRet); })? RightBracket
    | ddr=directDeclarator { $dd = new DirectDeclarator(); $dd.setDd($ddr.dd); } lp=LeftParen  (pl=parameterList { $dd.setPl($pl.plRet); }|
      (il=identifierList { $dd.setIl($il.il); })?) RightParen { $dd.setLine($lp.line); };
...
compoundStatement returns[CompoundStatement CSRet]
    : { $CSRet = new CompoundStatement(); } lb=LeftBrace ((bi=blockItem { $CSRet.addBi($bi.BIRet); })+)? RightBrace ;
